...
const SEEN_KEY = 'gingerDragonEntranceSeenV2';
...
.studio-cinema{position:absolute;inset:0;z-index:1;overflow:hidden;background:radial-gradient(circle at 50% 48%,#39291f 0,#171116 48%,#0b080d 100%);opacity:0;transition:opacity .5s ease}
...
.studio-door-frame{position:absolute;width:min(88vw,780px);height:min(90vh,920px);max-height:94%;top:50%;left:50%;transform:translate(-50%,-50%);box-shadow:0 0 0 20px #312a25,0 0 0 28px #181514,0 30px 100px #000;border-radius:40% 40% 2% 2%/18% 18% 2% 2%;overflow:hidden;background:#0e0b0d}
.studio-library{position:absolute;inset:0;z-index:0;background-image:linear-gradient(rgba(6,4,8,.08),rgba(6,4,8,.12)),url('${LIBRARY_ASSET}');background-position:50% 50%;background-size:cover;background-repeat:no-repeat;opacity:0;transform:scale(1.04);filter:saturate(.98) contrast(1.03) brightness(.92);transition:opacity 1s ease,transform 1.8s cubic-bezier(.2,.7,.2,1),filter 1.2s ease;background-position 1.6s ease;will-change:transform,opacity}
.studio-library::after{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% 47%,rgba(255,211,120,.05),transparent 28%),radial-gradient(ellipse at center,transparent 52%,rgba(4,2,6,.42) 100%);pointer-events:none}
.studio-door-half{position:absolute;z-index:2;top:0;bottom:0;width:50.2%;background:linear-gradient(90deg,#211b18,#514638 18%,#29221d 48%,#4c4034 80%,#1e1916);box-shadow:inset 0 0 45px #090707,0 0 24px rgba(0,0,0,.75);transition:transform 1.25s cubic-bezier(.65,.04,.28,1);will-change:transform}
...
.studio-door-seam{position:absolute;z-index:3;left:50%;top:7%;bottom:7%;width:3px;transform:translateX(-50%);background:#f6d994;box-shadow:0 0 9px #ffe6a4,0 0 30px #f2b957,0 0 70px rgba(255,190,86,.65);opacity:0;transition:opacity .55s ease}
.studio-entrance-root.studio-door-lit .studio-door-seam{opacity:1}
.studio-entrance-root.studio-door-open .studio-door-left{transform:translateX(-101%) rotateY(-8deg)}
.studio-entrance-root.studio-door-open .studio-door-right{transform:translateX(101%) rotateY(8deg)}
.studio-entrance-root.studio-door-open .studio-door-seam{opacity:0}
.studio-entrance-root.studio-library-visible .studio-library{opacity:1;transform:scale(1);filter:saturate(1) contrast(1.02) brightness(1)}
.studio-entrance-root.studio-tome-focus .studio-library{transform:scale(1.34);background-position:50% 45%;filter:saturate(1.03) contrast(1.03) brightness(1.08)}
.studio-entrance-root.studio-tome-focus .studio-library::after{background:radial-gradient(circle at 50% 43%,rgba(255,218,137,.16),transparent 24%),radial-gradient(ellipse at center,transparent 48%,rgba(4,2,6,.5) 100%)}
.studio-poster-stage{z-index:5!important;transition:transform 1.2s cubic-bezier(.22,.72,.18,1),filter 1.1s ease;will-change:transform}
...
function runCinema(root: HTMLElement, cta: HTMLButtonElement, nativeCta: HTMLElement | null, card: HTMLElement) {
  ...
  timers.push(window.setTimeout(() => root.classList.add('studio-door-lit'), 850));
  timers.push(window.setTimeout(() => root.classList.add('studio-door-open'), 1500));
  timers.push(window.setTimeout(() => root.classList.add('studio-library-visible'), 1900));
  timers.push(window.setTimeout(() => root.classList.add('studio-tome-focus'), 3900));
  timers.push(window.setTimeout(finish, 5900));
  timers.push(window.setTimeout(finish, 7200)); // hard fail-forward guard
}
