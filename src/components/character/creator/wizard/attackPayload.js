/* Canonical attack serialization for wizard-created crawlers.
   ...
   PLUS the selected starting weapon at Rank 3. Attack Spell path -> the
   baseline attack only; a chosen Attack Spell becomes a known spell
   (spellPayload.js), never a physical weapon attack.
...
export const canonicalAttacksFromDraft = (profile, draft) => {
  ...
  const hitBonus = (skillId, statKey, rank) => {
    ...
    const atkStat = (statKey ?? def.attack_stat ?? "").toLowerCase();
