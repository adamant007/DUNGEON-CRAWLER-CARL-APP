    ...
    <div className={`${CARD} flex flex-wrap items-center justify-between gap-2`}>
      <div>
        ...
        <p className="font-fell text-[11px] text-[var(--ink-soft)]">
          Base {math.base} BP
          {math.detrimentCredit ? ` + ${math.detrimentCredit} from drawbacks` : ""}
        </p>
      </div>
...
function StepTabs({ step, setStep }) {
  const labels = ["Concept", "Benefits", "Drawbacks", "Review"];
  return (
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
          ...
          <p className="mt-2 font-fell text-[11px] text-[var(--ink-soft)]">
            {build.kind === "race"
              ? build.earth
                ...
                : "Alien Races do not get access to Earth-only Classes."
              : build.earth
        ...
        {build.kind === "race" ? (
          <div className={CARD}>
            ...
            {build.size === "Small" ? (
              <p className="mt-2 font-fell text-[11px] text-[var(--hp)]">
                Small is a 3 BP Major Benefit. It is added automatically to the final build.
              </p>
            ) : null}
          </div>
        ) : (
          <label className={CARD}>
            ...
            <select
              ...
              value={build.classType}
              onChange={(e) => setBuild((b) => ({ ...b, classType: e.target.value }))}
            >
      ...
      <div className={CARD}>
        ...
        <p className="mt-1 font-fell text-[11px] text-[var(--ink-soft)]">
          Pick a starting direction. Ginger Dragon will make a legal starter package; you can change every choice afterward.
        </p>
      ...
      <label className="block">
        <span className="font-fell-sc text-[11px] font-bold tracking-[0.05em] text-[#24180f]">FUNNY / FLAVOR DESCRIPTION (FREE — NO BP)</span>
        <textarea
          className={INPUT + " mt-1 min-h-[82px]"}
          value={build.description}
          onChange={(e) => setBuild((b) => ({ ...b, description: e.target.value }))}
          placeholder="What kind of ridiculous Dungeon creation is this?"
        />
      </label>
    </div>
  );
...
function BenefitsStep({ build, setBuild, skills }) {
  ...
  const skillRows = Object.values(skills ?? {})
    .filter((s) => !["damage_effect"].includes(s.category))
    ...
    <div className="space-y-3">
      ...
      <div className={CARD}>
        <div className="flex flex-wrap items-end justify-between gap-2">
          <div>
            ...
            <p className="font-fell text-[11px] text-[var(--ink-soft)]">+1 Rank costs 2 BP. Rank-20 access costs 1 BP. Advantage on one non-combat Skill costs 3 BP.</p>
          </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="min-w-[170px] flex-1">
                    ...
                    <p className="font-fell text-[10px] text-[var(--ink-soft)]">
                      {skill.category || "Skill"}{skill.stat ? ` · ${String(skill.stat).toUpperCase()}` : ""}
                    </p>
                  </div>
                  ...
                  <label className="flex items-center gap-1 font-fell text-[10px] text-[#24180f]">
                    <input type="checkbox" checked={!!rank20} onChange={() => toggleArray("rank20", skill.id)} /> Rank 20
                  </label>
                  {canAdvantage ? (
                    <label className="flex items-center gap-1 font-fell text-[10px] text-[#24180f]">
                      <input type="checkbox" checked={!!advantage} onChange={() => toggleArray("advantageSkills", skill.id)} /> Advantage
                    </label>
                  ) : null}
                </div>
              </div>
      ...
      <div className={CARD}>
        ...
        <div className="mt-3">
          <CounterRow
            label="DR Buff"
            note="2 BP per +1, maximum +3 from the build."
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <label>
            ...
            <select className={INPUT + " mt-1"} value={build.uncommonResistance} onChange={(e) => setBuild((b) => ({ ...b, uncommonResistance: e.target.value }))}>
              ...
              {UNCOMMON_DAMAGE.map((v) => <option key={v}>{v}</option>)}
            </select>
          </label>
          <label>
            ...
            <select className={INPUT + " mt-1"} value={build.commonResistance} onChange={(e) => setBuild((b) => ({ ...b, commonResistance: e.target.value }))}>
              <option value="">None</option>
              {COMMON_DAMAGE.map((v) => <option key={v}>{v}</option>)}
            </select>
          </label>
        </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={CARD}>
        ...
        <p className="font-fell text-[11px] text-[var(--ink-soft)]">
          Every 2 total penalty points grants +1 BP. All drawbacks together can grant at most +5 BP.
        </p>
        {STAT_KEYS.map((key) => {
      ...
      <div className={CARD}>
        ...
        <label className="mt-3 block">
          ...
          <select className={INPUT + " mt-1"} value={build.statCap10} onChange={(e) => setBuild((b) => ({ ...b, statCap10: e.target.value }))}>
            <option value="">No Stat cap</option>
            {STAT_KEYS.map((key) => <option key={key} value={key}>{STAT_LABELS[key]}</option>)}
          </select>
        </label>
      </div>
    </div>
    ...
    <div className="space-y-3">
      ...
      <div className={CARD}>
        ...
        <div className="mt-2 grid gap-1 font-fell text-[12px] text-[#24180f] sm:grid-cols-2">
          {STAT_KEYS.filter((k) => Number(build.statBonuses[k]) > 0).map((k) => <span key={k}>+{build.statBonuses[k]} {STAT_LABELS[k]}</span>)}
          {STAT_KEYS.filter((k) => Number(build.statPenalties[k]) < 0).map((k) => <span key={k}>{build.statPenalties[k]} {STAT_LABELS[k]}</span>)}
      ...
      {!validation.legal ? (
        ...
        <div className="border border-[#557b49] bg-[rgba(85,123,73,0.08)] p-3">
          ...
          <p className="mt-1 font-fell text-[11px] text-[#24180f]">
            {math.benefitsSpent} BP spent out of {math.available}.
            {math.remaining > 0 ? ` ${math.remaining} BP will be unused.` : " Full budget used."}
          </p>
        </div>
      )}
...
export default function GuidedHomebrewBuilderDialog({ profile, savedRaces = [], savedClasses = [], onSave, onClose }) {
  ...
  // Size 2 is a Major benefit. Keep it explicit in the budget without
  // asking the player to know the price.
  const budgetBuild = React.useMemo(() => {
    if (build.kind !== "race" || build.size !== "Small") return build;
    const next = { ...build, specials: [...build.specials] };
    // Synthetic special handled only for budgeting/review; serializer uses size.
    return next;
  }, [build]);

  const mathWithSize = React.useMemo(() => {
    const baseMath = buildMath(budgetBuild);
    if (build.kind === "race" && build.size === "Small") {
      const benefitsSpent = baseMath.benefitsSpent + 3;
      return { ...baseMath, benefitsSpent, remaining: baseMath.available - benefitsSpent, legal: benefitsSpent > 0 && benefitsSpent <= baseMath.available && baseMath.rawDetrimentCredit <= 5 };
    }
    return baseMath;
  }, [budgetBuild, build.kind, build.size]);

  // Feed Small-size cost into the normal validator/serializer through a
  // hidden stateless marker by wrapping save validation here.
  const save = (entry) => {
    if (build.kind === "race" && build.size === "Small") {
      if (mathWithSize.remaining < 0) return;
      entry.build_points.benefits_spent += 3;
      entry.build_points.remaining -= 3;
    }
    onSave?.(entry);
  };
    ...
    <ParchmentDialog title="Guided Race / Class Builder" large onClose={onClose}>
      <div className="flex h-full min-h-0 flex-col gap-3">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          ...
          {step === 3 ? <ReviewStep build={build} skills={skills} onSave={save} /> : null}
        </div>
