...
function installCleanAppShell(output){
  const file=path.join(root,output);let source=fs.readFileSync(file,"utf8");
  ...
  if(appStart===-1)throw new Error('App component not found for clean shell launch bridge');
  const appEnd=source.indexOf('\ncreateRoot(',appStart);
  if(appEnd===-1)throw new Error('React root marker not found after App component');
  const appSource=source.slice(appStart,appEnd);
  const landingReturn=appSource.match(/if\(([^)]*)\)return <Landing onLaunch=\{\(\)=>setEntered\(true\)\}\/>;/);
  if(!landingReturn)throw new Error('Landing launch guard not found in App component');
  const guard=landingReturn[1];
  const cleanGuard=`if(${guard})return <Landing onLaunch={()=>setEntered(true)}/>;return <CleanAppShell/>;`;
  const replacedApp=appSource.replace(landingReturn[0],cleanGuard);
  source=source.slice(0,appStart)+replacedApp+source.slice(appEnd);
  fs.writeFileSync(file,source);console.log('Landing now launches the standalone clean Crawler Companion shell');
}
...
restore("main","src/main.tsx");restore("styles","src/styles.css");restoreBrandAsset();cleanLandingSource("src/main.tsx");makeCharacterPrimary("src/main.tsx");installTestCharacter("src/main.tsx");installCleanAppShell("src/main.tsx");injectLandingHeroStyles("src/styles.css");injectCoreIntegration("src/main.tsx");
