...
update public.campaigns set name=title where title is not null and btrim(title)<>'' and (name is null or btrim(name)='' or name='My Crawler Campaign');
...
create index if not exists campaign_events_target_idx on public.campaign_events(campaign_id,target_character_id,created_at desc);
...
-- Campaign event delivery: every member may receive broadcasts, but targeted events are
-- visible only to the GM/owner and to the account that owns the target character.
drop policy if exists "events member read" on public.campaign_events;
create policy "events member read" on public.campaign_events for select using(
  exists(select 1 from public.campaign_members m where m.campaign_id=campaign_events.campaign_id and m.user_id=auth.uid())
  and (
    target_character_id is null
    or exists(select 1 from public.campaigns c where c.id=campaign_events.campaign_id and c.owner_id=auth.uid())
    or exists(select 1 from public.crawler_characters ch where ch.id=campaign_events.target_character_id and ch.user_id=auth.uid())
  )
);

-- GM pushes (loot boxes, announcements, damage/healing, etc.) are authoritative. Players
-- cannot forge campaign events. A target character, when supplied, must belong to a member
-- of the same campaign.
drop policy if exists "events member insert" on public.campaign_events;
create policy "events gm insert" on public.campaign_events for insert with check(
  sender_id=auth.uid()
  and exists(select 1 from public.campaigns c where c.id=campaign_events.campaign_id and c.owner_id=auth.uid())
  and (
    target_character_id is null
    or exists(
      select 1 from public.crawler_characters ch
      join public.campaign_members m on m.user_id=ch.user_id
      where ch.id=campaign_events.target_character_id and m.campaign_id=campaign_events.campaign_id
    )
  )
);
...
begin
  ...
  return jsonb_build_object('total_accounts',(select count(*) from auth.users),'unique_app_openers',(select count(*) from public.app_visits),'active_users_today',(select count(*) from public.app_visits where last_seen>=date_trunc('day',now())),'characters_created',(select count(*) from public.crawler_characters),'active_campaigns',(select count(*) from public.campaigns),'campaign_events',(select count(*) from public.campaign_events));
end $$;
...
do $$ begin alter publication supabase_realtime add table public.campaign_events; exception when duplicate_object then null; end $$;
do $$ begin alter publication supabase_realtime add table public.crawler_characters; exception when duplicate_object then null; end $$;